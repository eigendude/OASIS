...
bool ImageDownscalerNode::Initialize()
{
  ...
  auto toOptionalDimension = [this](std::string_view parameterName, int64_t rawValue,
    ...
    if (rawValue > std::numeric_limits<unsigned int>::max())
    {
      RCLCPP_ERROR(m_node.get_logger(),
                   "Parameter '%s' value %ld exceeds supported range", parameterName.data(), rawValue);
      return false;
    }
  ...
  if (outputWidth || outputHeight)
  {
    ...
    RCLCPP_INFO(m_node.get_logger(), "Output dimensions: %sx%s", widthLog.c_str(), heightLog.c_str());
  }
  ...
  try
  {
    ...
    m_downscaler = std::make_unique<IMAGE::ImageDownscaler>(
        ...
        std::move(imageTransport), std::move(outputWidth), std::move(outputHeight), std::move(maxWidth),
        std::move(maxHeight));
  }
