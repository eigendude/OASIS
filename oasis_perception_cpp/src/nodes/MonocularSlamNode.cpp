    ...
    m_imgSubscriber(std::make_unique<image_transport::Subscriber>())
{
  ...
  m_node.declare_parameter<std::string>(IMAGE_TRANSPORT_PARAMETER.data(), DEFAULT_IMAGE_TRANSPORT);
  m_node.declare_parameter<std::string>(VOCABULARY_FILE_PARAMETER.data(), DEFAULT_VOCABULARY_FILE);
  m_node.declare_parameter<std::string>(SETTINGS_FILE_PARAMETER.data(), DEFAULT_SETTINGS_FILE);
}
...
bool MonocularSlamNode::Initialize()
{
  ...
  {
    RCLCPP_ERROR(
        *m_logger, "Missing or empty system ID parameter '%s'", SYSTEM_ID_PARAMETER.data());
    return false;
  }
