...
// Parameters for whether it should run and publish scene scores
constexpr std::string_view PUBLISH_SCENE_SCORES_PARAMETER = "publish_scene_scores";
constexpr bool DEFAULT_PUBLISH_SCENE_SCORES = true;
} // namespace
...
bool OpticalFlowNode::Initialize()
{
  ...
  {
    RCLCPP_ERROR(m_node.get_logger(), "Missing scene score publish parameter '%s'",
  ...
  RCLCPP_INFO(m_node.get_logger(), "Publishing scene scores: %s", m_publishSceneScores ? "true" : "false");
...
void OpticalFlowNode::OnImage(const sensor_msgs::msg::Image::ConstSharedPtr& msg)
{
  ...
  RCLCPP_DEBUG(m_node.get_logger(), "Tracked %zu optical flow points", trackedPointCount);
