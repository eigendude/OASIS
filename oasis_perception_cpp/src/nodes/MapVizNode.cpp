...
bool MapVizNode::Initialize()
{
  ...
  *m_mapImagePublisher = image_transport::create_publisher(&m_node, mapImageTopic);
  ...
  m_poseSubscription = m_node.create_subscription<PoseStamped>(
      poseTopic, {1}, [this](const PoseStamped::ConstSharedPtr& msg) { OnPose(msg); });

  m_pointCloudSubscription = m_node.create_subscription<PointCloud2>(
      pointCloudTopic, {1}, [this](const PointCloud2::ConstSharedPtr& msg) { OnPointCloud(msg); });

  m_aprilTagSubscription = m_node.create_subscription<AprilTagDetectionArray>(
      aprilTagTopic, {1},
      [this](const AprilTagDetectionArray::ConstSharedPtr& msg) { OnAprilTags(msg); });

  m_cameraInfoSubscription = m_node.create_subscription<CameraInfo>(
      cameraInfoTopic, {1}, [this](const CameraInfo::ConstSharedPtr& msg) { OnCameraInfo(msg); });
