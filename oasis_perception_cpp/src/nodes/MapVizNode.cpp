    ...
    m_mapImagePublisher(std::make_unique<image_transport::Publisher>())
{
  ...
  m_node.declare_parameter<std::string>(SETTINGS_FILE_PARAMETER.data(), DEFAULT_SETTINGS_FILE.data());
}
...
bool MapVizNode::Initialize()
{
  ...
  {
    RCLCPP_ERROR(m_logger, "Failed to load camera model from settings file: %s", settingsFile.c_str());
    return false;
  }
  ...
  *m_mapImagePublisher =
  ...
  m_pointCloudSubscription = m_node.create_subscription<PointCloud2>(
      pointCloudTopic, sensorQos, std::bind(&MapVizNode::OnPointCloud, this, std::placeholders::_1));
...
Eigen::Isometry3f MapVizNode::PoseMsgToIsometry(const PoseStamped& poseMsg)
{
  ...
  transform.translation() = Eigen::Vector3f(static_cast<float>(position.x),
                                            static_cast<float>(position.y),
                                            static_cast<float>(position.z));

  const auto& orientation = poseMsg.pose.orientation;
  Eigen::Quaternionf quaternion(static_cast<float>(orientation.w), static_cast<float>(orientation.x),
