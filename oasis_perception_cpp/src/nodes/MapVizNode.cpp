...
void MapVizNode::OnDetections(const apriltag_msgs::msg::AprilTagDetectionArray::ConstSharedPtr& msg)
{
  if (msg == nullptr)
    return;
...
void MapVizNode::OnPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg)
{
  ...
  const double backgroundDeltaSeconds = std::abs((pointCloudTime - backgroundTime).seconds());
  ...
  apriltag_msgs::msg::AprilTagDetectionArray::ConstSharedPtr detectionsMsg;
  {
    ...
    if (m_latestDetectionsMsg != nullptr)
    {
      ...
      const double detectionsDeltaSeconds = std::abs((pointCloudTime - detectionsTime).seconds());
