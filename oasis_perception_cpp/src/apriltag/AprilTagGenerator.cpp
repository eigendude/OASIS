...
cv::Mat AprilTagGenerator::Generate(const std::string& family, int32_t id, int scale)
{
  ...
  if (id < 0 || id >= apriltagFamily->ncodes)
  {
    RCLCPP_WARN(m_logger,
                "AprilTag ID %d is out of range for family %s (max id: %d)",
                id,
                family.c_str(),
                apriltagFamily->ncodes - 1);
    return {};
  }
...
std::string AprilTagGenerator::NormalizeFamily(const std::string& family)
{
  ...
  std::transform(familyLower.begin(), familyLower.end(), familyLower.begin(), [](unsigned char c) {
    return static_cast<char>(std::tolower(c));
  });
  return familyLower;
}

std::optional<AprilTagGenerator::FamilyFactory>
AprilTagGenerator::GetFamilyFactory(const std::string& family)
{
