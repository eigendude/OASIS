...
void MapViewRenderer::UpdateRoiMask()
{
  ...
  if (!m_roiMaskDirty && !m_roiMask.empty() && m_roiMask.cols == m_width && m_roiMask.rows == m_height)
    return;
  ...
  if (polygon.empty())
  {
    const int cutoff = std::max(1, static_cast<int>(std::round(0.28f * static_cast<float>(m_width))));
