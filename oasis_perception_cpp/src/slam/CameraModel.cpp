...
bool ValidateCameraModel(const CameraModel& model)
{
  return model.width > 0 && model.height > 0 &&
         std::isfinite(model.fx) && model.fx > 0.0f &&
         std::isfinite(model.fy) && model.fy > 0.0f &&
         std::isfinite(model.cx) && std::isfinite(model.cy);
}

} // namespace

bool SLAM::LoadCameraModel(const std::string& settingsFile, CameraModel& model,
                           rclcpp::Logger& logger)
{
  ...
  {
    RCLCPP_ERROR(logger,
                 "Invalid camera model read from settings file %s (width=%d height=%d fx=%f fy=%f cx=%f cy=%f)",
                 settingsFile.c_str(), model.width, model.height, model.fx, model.fy, model.cx, model.cy);
    return false;
  }

  RCLCPP_INFO(logger,
              "Loaded camera model: width=%d height=%d fx=%.3f fy=%.3f cx=%.3f cy=%.3f",
