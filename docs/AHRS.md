...
Define `Δt_sec := Δt_ns * 1e-9` (deterministic scale; not used for keying).
...
- `t_meas_ns`: `header.stamp` from the incoming message (authoritative event time)
...
- If there is a timestamp gap larger than `Δt_imu_max_ns` between consecutive nodes required to cover replay,
...
4. Update frontier / replay
   - if `t_meas_ns > t_filter_ns`: advance `t_filter_ns` and propagate forward
   - else: apply update at `t_meas_ns`, then replay forward to `t_filter_ns`
...
- propagation coverage insufficient (`Δt_imu_max_ns` violated)
- clock discontinuity detected: reset filter + buffer if `|t_meas_ns - t_filter_ns| > Δt_clock_jump_max_ns`
  - Reset effects (explicit):
    - clear the time buffer and set `t_filter_ns` to unset
...
- Let `t_filter_prev_ns` be the frontier before processing an incoming message.
- After processing a message:
  - If `t_meas_ns > t_filter_prev_ns` (frontier advance), the filter propagates to `t_meas_ns`, applies updates at `t_meas_ns`,
    sets `t_filter_ns := t_meas_ns`, and **publishes**:
      - TF (`world -> odom`, `odom -> base_link`) stamped `t_filter_ns`
      - odometry outputs stamped `t_filter_ns`
      - `oasis_msgs/AhrsState` stamped `t_filter_ns`
      - `oasis_msgs/AhrsDiagnostics` stamped `t_filter_ns`
  - If `t_meas_ns <= t_filter_prev_ns` (out-of-order insertion), the filter may replay internally to maintain consistency at
    `t_filter_ns`, but **does not republish** TF/odometry/state/diagnostics (to avoid emitting past timestamps).

Per-measurement update reports (`oasis_msgs/EkfUpdateReport`) are published at the measurement timestamp `t_meas_ns` for
...
- Broadcast `TF: odom -> base_link` from `T_OB` (`stamp = t_filter_ns`)
- Broadcast `TF: world -> odom` as identity (`stamp = t_filter_ns`)
...
Each per-measurement topic publishes `oasis_msgs/EkfUpdateReport` stamped at `t_meas_ns` for every accepted/rejected update.
