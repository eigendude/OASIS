...
def test_process_noise_coefficients() -> None:
    ...
    for axis_index in range(axis_count):
        ...
        assert math.isclose(
            noise[pos_index][pos_index], pos_noise, abs_tol=1.0e-12
        )
        assert math.isclose(
            noise[pos_index][vel_index], pos_vel_noise, abs_tol=1.0e-12
        )
        assert math.isclose(
            noise[vel_index][pos_index], pos_vel_noise, abs_tol=1.0e-12
        )
