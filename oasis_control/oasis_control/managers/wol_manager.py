...
class WolManager:
    ...
    def initialize(self, service_timeout_secs: Optional[float] = None) -> bool:
        ...
        if not self._get_mac_address_client.wait_for_service(service_timeout_secs):
            self._logger.error(f"Failed to connect to get_mac_address service after {service_timeout_secs}s")
            return False

        self._logger.debug("  - Waiting for wol...")
        if not self._wol_client.wait_for_service(service_timeout_secs):
            self._logger.error(f"Failed to connect to wol service after {service_timeout_secs}s")
            return False
    ...
    def send_wol(self) -> bool:
        if self._mac_address is None:
            # Wait for result
            rclpy.spin_until_future_complete(self._node, self._mac_address_future)
            if self._mac_address_future.result() is None:
                self._logger.error(f"Exception while calling service: {self._mac_address_future.exception()}")
                return False
