...
from oasis_control.localization.ekf.ekf_types import (
    AprilTagDetection,
...
class EkfLocalizerNode(rclpy.node.Node):
    def __init__(self) -> None:
        ...
        self._world_frame_id: str = str(
            self.get_parameter(PARAM_WORLD_FRAME_ID).value
        )
        ...
        self._tag_anchor_id: int = int(
            self.get_parameter(PARAM_TAG_ANCHOR_ID).value
        )
    ...
    def _handle_apriltags(self, message: AprilTagDetectionArrayMsg) -> None:
        ...
        if self._camera_info is None:
            ...
            self.get_logger().warn(
                "AprilTag frame does not match cached camera_info"
            )
    ...
    def _publish_odom(self, timestamp: float) -> None:
        message: Odometry = self._build_odom(timestamp)
    ...
    def _publish_mag_update(self, update: EkfUpdateData) -> None:
        ...
        report: EkfUpdateReport = self._reporter.build_update_report(
            update, update_seq, 0
        )
    ...
    def _publish_apriltag_update(self, update: EkfAprilTagUpdateData) -> None:
        update_seq: int = self._next_update_seq()
        report: EkfAprilTagUpdateReport = self._reporter.build_apriltag_report(
            update, update_seq
        )
    ...
    def _publish_rejection(self, event: EkfEvent, reason: str) -> None:
        if event.event_type == EkfEventType.MAG:
            mag_sample: MagSample = event.payload
            ...
            apriltag_data: AprilTagDetectionArrayData = event.payload
            update_data: EkfAprilTagUpdateData = (
                self._core.update_with_apriltags(apriltag_data, event.t_meas)
            )
    ...
    def _build_odom(self, timestamp: float) -> Odometry:
    ...
    def _camera_info_to_data(self, message: CameraInfo) -> CameraInfoData:
