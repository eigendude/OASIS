    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self.get_logger().error(
                f"Failed to create mount info directory at {mount_info_dir}: {exc}"
            )
        ...
        self.get_logger().info(
            f"AHRS mount calibration file {'found' if calibration_exists else 'NOT found'}: {calibration_path}"
        )
        ...
        if not self._params.save.output_path:
            ...
            try:
                ...
                self.get_logger().error(
                    f"Failed to load mounting calibration from {self._mount_calibration_path}: {exc}"
                )
                raise RuntimeError(
