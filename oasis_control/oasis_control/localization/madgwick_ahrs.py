...
class MadgwickAhrs:
    ...
    def update_imu(
        ...
        q_dot1: float = (
            -half_rate * (q2 * gx + q3 * gy + q4 * gz) - self._beta * s1
        )
        q_dot2: float = (
            half_rate * (q1 * gx + q3 * gz - q4 * gy) - self._beta * s2
        )
        q_dot3: float = (
            half_rate * (q1 * gy - q2 * gz + q4 * gx) - self._beta * s3
        )
        q_dot4: float = (
            half_rate * (q1 * gz + q2 * gy - q3 * gx) - self._beta * s4
        )
