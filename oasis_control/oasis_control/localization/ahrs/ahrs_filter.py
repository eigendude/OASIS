...
class AhrsFilter:
    ...
    def _apply_event(self, event: AhrsEvent) -> None:
        if event.event_type == AhrsEventType.IMU:
            ...
            self._maybe_initialize_gravity(payload_imu.imu)
        ...
        if event.event_type == AhrsEventType.MAG:
            ...
            self._maybe_initialize_magnetic_field(payload_mag)
    ...
    def _maybe_initialize_gravity(self, imu: ImuSample) -> None:
        if self._x is None:
            return

        if _norm3(self._x.g_w_mps2) >= _EPS_NORM:
            return
        ...
        # m/s^2 gravity magnitude derived from stationary accel norm
        g_mag: float = accel_norm
        self._x.g_w_mps2 = [0.0, 0.0, -g_mag]

        accel_unit: list[float] = _scale3(accel, 1.0 / accel_norm)
        gravity_unit: list[float] = [0.0, 0.0, -1.0]
        self._x.q_wb_wxyz = _quat_from_two_unit_vectors(gravity_unit, accel_unit)

    def _maybe_initialize_magnetic_field(self, mag: MagSample) -> None:
        if self._x is None:
            return

        if _norm3(self._x.m_w_t) >= _EPS_NORM:
            return
        ...
        q_wb: list[float] = list(self._x.q_wb_wxyz)
        if len(q_wb) != 4 or not is_finite_seq(q_wb):
            return

        q_bw: list[float] = quat_conj_wxyz(q_wb)
        magnetic_world: list[float] = quat_rotate_wxyz(q_bw, magnetic)
        if not is_finite_seq(magnetic_world):
            return

        self._x.m_w_t = magnetic_world
...
def _quat_from_two_unit_vectors(
    ...
    quat: list[float] = [1.0 + dot, cross[0], cross[1], cross[2]]
    return quat_normalize_wxyz(quat)
