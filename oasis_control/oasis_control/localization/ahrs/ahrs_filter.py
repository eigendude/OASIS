...
from oasis_control.localization.ahrs.ahrs_bootstrap import maybe_initialize_gravity
from oasis_control.localization.ahrs.ahrs_bootstrap import (
    maybe_initialize_magnetic_field,
)
...
class AhrsFilter:
    ...
    def _maybe_initialize_gravity(self, imu: ImuSample) -> None:
        if self._x is None:
            return
        maybe_initialize_gravity(self._x, imu)

    def _maybe_initialize_magnetic_field(self, mag: MagSample) -> None:
        if self._x is None:
            return
        maybe_initialize_magnetic_field(self._x, mag)
