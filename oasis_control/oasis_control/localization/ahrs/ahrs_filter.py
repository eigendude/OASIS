...
class AhrsFilter:
    ...
    def _has_nan_covariance(self, event: AhrsEvent) -> bool:
        ...
        if event.event_type == AhrsEventType.IMU:
            payload = cast(AhrsImuPacket, event.payload)
            covariances = (
                payload.imu.angular_velocity_cov
                + payload.imu.linear_acceleration_cov
            )
        elif event.event_type == AhrsEventType.MAG:
            payload = cast(MagSample, event.payload)
            covariances = payload.magnetic_field_cov
