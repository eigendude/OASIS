...
class VelocityEstimator:
    ...
    def update(
        ...
        if dt <= 0.0 or dt > self._max_dt_spike_s:
            ...
            v_body_skip, cov_v_body_skip = self._to_body_frame(
                quat_body_to_world
            )
            return v_body_skip, cov_v_body_skip, self._is_stationary()

        quat_body_to_world: np.ndarray = self._normalize_quaternion(
            quat_body_to_world
        )
        ...
        cov_accel_body: np.ndarray = self._validate_matrix(
            cov_accel_body,
            "cov_accel_body",
        )
        cov_theta_body: np.ndarray = self._validate_matrix(
            cov_theta_body,
            "cov_theta_body",
        )

        rotation_body_to_world: np.ndarray = self._quat_to_rotmat(quat_body_to_world)
        ...
        cov_accel_world: np.ndarray = (
            rotation_body_to_world @ cov_accel_body @ rotation_body_to_world.T
        )

        # Rotate attitude covariance from body frame into world frame
        cov_theta_world: np.ndarray = (
            rotation_body_to_world @ cov_theta_body @ rotation_body_to_world.T
        )
