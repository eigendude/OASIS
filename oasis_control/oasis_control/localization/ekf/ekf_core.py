...
class EkfCore:
    ...
    def update_with_apriltag_pose(
        ...
        detection_update: EkfAprilTagDetectionUpdate = (
            self._update_with_apriltag_pose(apriltag_pose, t_meas)
        )
    ...
    def odom_base_pose(self) -> EkfPose:
        ...
        q_world_base: np.ndarray = np.asarray(
            world_base.orientation_xyzw, dtype=float
        )
    ...
    def _update_world_odom_from_odom_base(
        ...
        p_base_odom, q_base_odom = self._transform_inverse(
            p_odom_base, q_odom_base
        )
