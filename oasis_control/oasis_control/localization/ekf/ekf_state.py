...
class EkfState:
    ...
    def apply_delta(self, delta: np.ndarray) -> None:
        ...
        self.imu_bias_accel_mps2 = self.imu_bias_accel_mps2 + delta_vec[
            self._index.accel_bias
        ]
        accel_a_delta: np.ndarray = delta_vec[self._index.accel_a].reshape(3, 3)
        self.imu_accel_a = self.imu_accel_a + accel_a_delta
        self.imu_bias_gyro_rps = self.imu_bias_gyro_rps + delta_vec[
            self._index.gyro_bias
        ]
    ...
    def _build_initial_covariance(self) -> np.ndarray:
        ...
        covariance[self._index.pose.start, self._index.pose.start] = self._config.pos_var
        ...
        covariance[self._index.accel_bias, self._index.accel_bias] = (
            np.eye(3, dtype=float) * (_DEFAULT_ACCEL_BIAS_SIGMA_MPS2**2)
        )
        covariance[self._index.accel_a, self._index.accel_a] = (
            np.eye(9, dtype=float) * (_DEFAULT_ACCEL_A_SIGMA**2)
        )
        covariance[self._index.gyro_bias, self._index.gyro_bias] = (
            np.eye(3, dtype=float) * (_DEFAULT_GYRO_BIAS_SIGMA_RPS**2)
        )
