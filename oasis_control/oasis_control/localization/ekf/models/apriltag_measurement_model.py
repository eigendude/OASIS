...
# Minimum depth in meters for a valid projection
...
_SUPPORTED_DISTORTION_MODELS: set[str] = {
    "",
    "none",
    "plumb_bob",
    "rational_polynomial",
...
class AprilTagMeasurementModel:
    ...
    def set_camera_info(self, camera_info: CameraInfoData) -> None:
        ...
        self._fx = float(k_matrix[0, 0])
        self._fy = float(k_matrix[1, 1])
        self._cx = float(k_matrix[0, 2])
        self._cy = float(k_matrix[1, 2])
        self._skew = float(k_matrix[0, 1])
        self._distortion_model = camera_info.distortion_model.lower()
        self._distortion_coeffs = np.asarray(camera_info.d, dtype=float)
    ...
    def reprojection_residuals(
        ...
        tag_pose_c_array: np.ndarray
        try:
            tag_pose_c_array = np.asarray(tag_pose_c, dtype=float).reshape(-1)
        except ValueError:
            return None
    ...
    def _distort_points(
        ...
        model: str = self._distortion_model or ""
        coeffs: np.ndarray = (
        ...
        if not model or model == "none" or coeffs.size == 0:
            return x_norm, y_norm

        if model == "plumb_bob":
            return self._distort_plumb_bob(x_norm, y_norm, coeffs.tolist())

        if model == "rational_polynomial":
            return self._distort_rational_polynomial(x_norm, y_norm, coeffs.tolist())
