...
class EkfCoreAprilTagMixin(EkfCoreStateMixin, EkfCoreUtilsMixin):
    ...
    def _apply_odom_continuity_reset(
        ...
        if len(reset_indices) != 12:
            raise ValueError("Expected 12 reset indices for odom continuity")
        if len(set(reset_indices)) != len(reset_indices):
            raise ValueError("Expected reset indices to be unique")
        ...
        is_symmetric: bool = np.allclose(
            self._state.covariance,
            self._state.covariance.T,
            atol=1.0e-12,
        )
        if not is_symmetric:
            raise ValueError("Odom reset covariance lost symmetry")
        min_eigenvalue: float = float(
            np.min(np.linalg.eigvalsh(self._state.covariance))
        )
        if min_eigenvalue < -1.0e-9:
            raise ValueError("Odom reset covariance lost positive semidefiniteness")
