...
class EkfBuffer:
    ...
    def too_old(self, t_meas: EkfTime) -> bool:
        ...
        buffer_ns: int = int(self._config.t_buffer_sec * 1.0e9)
    ...
    def detect_clock_jump(self, t_meas: EkfTime) -> bool:
        ...
        jump_ns: int = int(self._config.dt_clock_jump_max * 1.0e9)
        return abs(dt_ns) > jump_ns

    def evict(self, t_filter: EkfTime) -> None:
        t_filter_ns: int = to_ns(t_filter)
        buffer_ns: int = int(self._config.t_buffer_sec * 1.0e9)
        cutoff_ns: int = t_filter_ns - buffer_ns
        index: int = 0
        while index < len(self._events) and self._timestamps_ns[index] < cutoff_ns:
            index += 1
        if index > 0:
    ...
    def iter_events_from(self, t_start_ns: int) -> Iterator[EkfEvent]:
        start_index: int = bisect_left(self._timestamps_ns, t_start_ns)
