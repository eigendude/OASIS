...
std::array<double, 3> NoiseEstimator::Update(int16_t x,
                                             int16_t y,
                                             int16_t z,
                                             double dt_s)
{
  const std::array<int32_t, 3> sample_counts{
      static_cast<int32_t>(x), static_cast<int32_t>(y),
      static_cast<int32_t>(z)};
  const double dt_clamped = std::clamp(dt_s, kMinDtSeconds, kMaxDtSeconds);
  const double prev_dt_clamped = std::clamp(m_prev_dt_s, kMinDtSeconds,
                                            kMaxDtSeconds);
  ...
  if (m_samples >= 2)
  {
    ...
    const double variance_denominator =
        use_uniform ? kUniformSecondDiffVarianceDenominator
                    : (a * a + b * b + 1.0);
    ...
    else
    {
      ...
      {
        m_sigma2_sum[axis] += sigma2_counts[axis] -
                              m_sigma2_window[axis][m_sigma2_index];
        m_sigma2_window[axis][m_sigma2_index] = sigma2_counts[axis];
      }
    }
  ...
  if (m_sigma2_count > 0)
  {
    ...
    {
      sigma2_mean[axis] = m_sigma2_sum[axis] /
                          static_cast<double>(m_sigma2_count);
    }
  }
