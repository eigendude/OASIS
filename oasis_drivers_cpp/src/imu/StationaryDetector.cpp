...
StationaryDetector::Status StationaryDetector::Update(const ImuSample& sample, const Noise& noise)
{
  ...
  for (std::size_t axis = 0; axis < 3; ++axis)
  {
    const double accel_var_gate =
        std::max(noise.accel_cov_mps2_2[axis][axis], accel_var_floor) *
        m_cfg.variance_sigma_mult;

    const double gyro_var_gate =
        std::max(noise.gyro_cov_rads2_2[axis][axis], gyro_var_floor) *
        m_cfg.variance_sigma_mult;
  ...
  {
    const double sigma =
        std::sqrt(std::max(noise.gyro_cov_rads2_2[axis][axis], gyro_var_floor));
    ...
    const double sigma_mean =
        std::max(sigma / std::sqrt(std::max(mean_count_floor, n)),
