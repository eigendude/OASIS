...
void ImuProcessor::UpdatePoseCluster(std::size_t idx, const Vec3& mean_accel_mps2)
{
  ...
  cluster.mean_accel_mps2 = AddScaled(cluster.mean_accel_mps2,
                                      Subtract(mean_accel_mps2, cluster.mean_accel_mps2), weight);
        ...
        for (const PoseCluster& cluster : m_poseClusters)
        {
          ...
          if constexpr (OASIS_IMU_ACCEL_CAL_DEBUG)
          {
            ...
            std::cerr << "[AccelCal] Cluster dir=[" << cluster.dir_unit[0] << ", "
                      << cluster.dir_unit[1] << ", " << cluster.dir_unit[2]
                      << "] count=" << cluster.count << " weight=" << weight << "\n";
          }
        }
