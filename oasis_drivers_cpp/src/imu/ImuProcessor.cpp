                                          ...
                                          double stamp_s)
{
  ...
  const Vec3 gyro_counts{static_cast<double>(gx), static_cast<double>(gy),
                         static_cast<double>(gz)};

  const Vec3 accel_mps2 =
      ScaleCounts(accel_counts, m_cfg.accel_scale_mps2_per_count);
  const Vec3 gyro_rads =
      ScaleCounts(gyro_counts, m_cfg.gyro_scale_rads_per_count);

  const ImuTemperature::Sample temp_sample =
      m_temperature.ProcessRaw(tempRaw, dt_clamped);
  ...
  {
    raw_sample.accel_cov_mps2_2 =
        m_calibrationRecord.measurement_noise.accel_cov_mps2_2;
    raw_sample.gyro_cov_rads2_2 =
        m_calibrationRecord.measurement_noise.gyro_cov_rads2_2;

    out.raw = raw_sample;
    out.has_raw = true;

    ImuSample corrected_sample = raw_sample;
    const Vec3 accel_unbiased =
        Subtract(accel_mps2, m_calibrationRecord.calib.accel_bias_mps2);
    corrected_sample.accel_mps2 =
        Multiply(m_calibrationRecord.calib.accel_A, accel_unbiased);
    corrected_sample.gyro_rads =
        Subtract(gyro_rads, m_calibrationRecord.calib.gyro_bias_rads);
  ...
  const double accel_scale2 =
      m_cfg.accel_scale_mps2_per_count * m_cfg.accel_scale_mps2_per_count;
  const double gyro_scale2 =
      m_cfg.gyro_scale_rads_per_count * m_cfg.gyro_scale_rads_per_count;
  ...
  const Vec3 gyro_var_rads2_2{gyro_var_counts2[0] * gyro_scale2,
                             gyro_var_counts2[1] * gyro_scale2,
  ...
  const StationaryDetector::Status status =
      m_stationaryDetector.Update(raw_sample, noise);
