    ...
    const Mpu6050ImuProcessor::ImuSample& sample)
{
  ...
  std::array<double, 3> gravityBlend{
      (1.0 - alphaG) * m_gravityHat[0] + alphaG * sample.accel[0],
  ...
  if (m_covSamples > 0)
  {
    ...
    {
      axisCandidate = (m_cov00 >= m_cov11) ? std::array<double, 2>{1.0, 0.0}
    ...
    if (!m_axisLocked && ratio > EIGEN_RATIO_THRESH && m_covSamples >= MIN_COV_SAMPLES && !m_stationary)
    {
      m_lockAccumTime += dt;
