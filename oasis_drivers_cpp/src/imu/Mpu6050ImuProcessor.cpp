...
std::optional<Mpu6050ImuProcessor::Output> Mpu6050ImuProcessor::Process(
    const RawSample& sample, const MotorIntentInput& motor)
{
  ...
  for (size_t i = 0; i < 3; ++i)
    m_mahonyIntegral[i] = std::clamp(m_mahonyIntegral[i], -m_config.mahony_integral_limit,
  ...
  if (m_isStationary && accelConfidence > 0.7)
  {
    ...
    for (size_t i = 0; i < 3; ++i)
    {
      m_gyroBiasVar[i] = std::clamp(m_gyroBiasVar[i] * (1.0 - alphaBias), m_config.bias_var_min,
  ...
  else
  {
    const double tempScale =
        m_hasTemperature
            ? 1.0 + m_config.temp_scale * std::abs(m_lastTemperature - m_temperatureAtCal)
            : 1.0;
    for (size_t i = 0; i < 3; ++i)
    {
      m_gyroBiasVar[i] = std::clamp(m_gyroBiasVar[i] + tempScale * m_config.bias_q * dt,
                                    m_config.bias_var_min, m_config.bias_var_max);
    }
  }
  ...
  if (m_isStationary && accelConfidence > 0.7)
  {
    ...
    for (size_t i = 0; i < 3; ++i)
    {
      ...
      EwmaUpdate(accelResidual, alpha, m_accelResidualMean[i], m_accelVar[i], m_accelVarInit[i]);
    }
    const Vec3 euler = EulerFromQuat(q);
  ...
  if (m_zUpSolved && !m_forwardSolved && forwardPlaneOk)
  {
    ...
    const bool hasResponse = accelLinXY > m_config.forward_response_accel_thresh ||
    ...
    if (event && hasResponse && !gyroVeto)
    {
      ...
      const bool dominantEligible = dominantAxis != 0 &&
                                    dominantConf > m_config.forward_score_thresh &&
  ...
  output.orientation_covariance = {rollVarPub, 0.0, 0.0, 0.0, pitchVarPub, 0.0, 0.0, 0.0, m_yawVar};
