    ...
    int16_t ax, int16_t ay, int16_t az, int16_t gx, int16_t gy, int16_t gz, double dt_seconds)
{
  ...
  if (sample.diag.stationary)
  {
    ...
    if (accel_norm > kEps)
    {
      ...
      {
        m_u_hat = Math::Normalize(Math::Add(Math::Scale(m_u_hat, 1.0 - kUpAlpha),
                                            Math::Scale(u_meas, kUpAlpha)));
      }
    }
  }
  ...
  sample.forward_diag =
      m_forwardAxisLearner.Update(sample.accel_mps2, sample.gyro_rads, dt_seconds, m_u_hat,
                                  m_u_hat_valid);
