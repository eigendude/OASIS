    ...
    const Mpu6050ImuProcessor::ImuSample& sample)
{
  ...
  if (m_calState == CalibrationState::kGravityCollect)
  {
    if (m_stationary)
    {
      ...
      if (m_gravityCollectTime >= T_GRAVITY_COLLECT &&
          m_gravitySamples >= MIN_GRAVITY_SAMPLES)
      {
  ...
  if (m_calState == CalibrationState::kYawPulseCollect)
  {
    ...
    if (!m_axisLocked && m_collectingYaw)
    {
      ...
      if (m_forwardSamples >= MIN_YAW_SAMPLES && m_yawConfidence > YAW_CONF_THRESH)
      {
        ...
        RCLCPP_INFO(logger,
                    "IMU calibration: calibrated q_s2t=(w=%.3f, x=%.3f, y=%.3f, z=%.3f)",
  ...
  if (m_calState != CalibrationState::kCalibrated)
  {
    RCLCPP_INFO_THROTTLE(
        ...
        static_cast<int>(m_calState), m_stationary ? "true" : "false", m_stillTime, m_moveTime,
        m_gyroEma, m_accelDevEma, m_gravityHatSolved[0], m_gravityHatSolved[1],
        m_gravityHatSolved[2], m_forwardDynamicEma, m_yawConfidence);
  }
