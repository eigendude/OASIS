...
{
  const std::array<int32_t, 3> sample_counts{static_cast<int32_t>(x),
  ...
  if (m_samples >= 2)
  {
    ...
    {
      const int32_t d2 =
          sample_counts[axis] - 2 * m_prev1_counts[axis] + m_prev2_counts[axis];
      const double d2_value = static_cast<double>(d2);
      const double sigma2_instant =
          (d2_value * d2_value) / kSecondDiffVarianceDenominator;
    ...
    int16_t ax, int16_t ay, int16_t az, int16_t gx, int16_t gy, int16_t gz)
{
  ...
  sample.gyro_rads = {static_cast<double>(gx) * m_gyroScale,
  ...
  for (size_t axis = 0; axis < accel_sigma2_counts.size(); ++axis)
  {
    ...
    sample.accel_variance_mps2[axis] =
        accel_total_counts * (m_accelScale * m_accelScale);
    sample.gyro_variance_rads[axis] =
        gyro_total_counts * (m_gyroScale * m_gyroScale);
  }
