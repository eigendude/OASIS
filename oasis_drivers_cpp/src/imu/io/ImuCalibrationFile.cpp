...
bool ImuCalibrationFile::Load(const std::filesystem::path& path, ImuCalibrationRecord& out) const
{
  ...
  if (!RequireScalar(calib["valid"]))
    return false;
  out.calib.valid = calib["valid"].as<bool>();
  ...
  if (!ReadMat3RowMajor(noise["accel_cov_raw_mps2_2"], out.measurement_noise.accel_cov_raw_mps2_2))
    return false;

  if (!ReadMat3RowMajor(noise["gyro_cov_raw_rads2_2"], out.measurement_noise.gyro_cov_raw_rads2_2))
    return false;
                              ...
                              const ImuCalibrationRecord& rec) const
{
  ...
  YAML::Node noise;
  noise["accel_cov_raw_mps2_2"] = WriteMat3RowMajor(rec.measurement_noise.accel_cov_raw_mps2_2);
  noise["gyro_cov_raw_rads2_2"] = WriteMat3RowMajor(rec.measurement_noise.gyro_cov_raw_rads2_2);
