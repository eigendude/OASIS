...
bool Mpu6050Node::Initialize()
{
  ...
  if (accelForcedAfsSel != 0 || accelForcedHpf != 0)
  {
    RCLCPP_WARN(get_logger(), "MPU6050 ACCEL_CONFIG readback AFS_SEL=%u ACCEL_HPF=%u (expected 0)",
                static_cast<unsigned>(accelForcedAfsSel), static_cast<unsigned>(accelForcedHpf));
  }
  ...
  for (int i = 0; i < 5; ++i)
  {
    ...
    uint8_t rawBuffer[6] = {};
    const int8_t accelBytesRead =
        I2Cdev::readBytes(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_ACCEL_XOUT_H, 6, rawBuffer);
    if (accelBytesRead != 6)
    {
    ...
    const double accelNormLsb = std::sqrt(
        static_cast<double>(ax) * ax + static_cast<double>(ay) * ay + static_cast<double>(az) * az);
    ...
    {
      RCLCPP_WARN(get_logger(), "MPU6050 startup ACCEL_CONFIG AFS_SEL=%u ACCEL_HPF=%u (expected 0)",
