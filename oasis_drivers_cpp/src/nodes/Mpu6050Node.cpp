...
Vec3 EulerFromQuat(const Quaternion& q)
{
  ...
  const double pitch = std::abs(sinp) >= 1.0 ? std::copysign(M_PI / 2.0, sinp)
                                             : std::asin(sinp);
...
Mpu6050Node::Mpu6050Node() : rclcpp::Node(NODE_NAME)
{
  ...
  declare_parameter("conductor_state_topic", std::string("/oasis/station/conductor_state"));
...
void Mpu6050Node::PublishImu()
{
  ...
  {
    const Quaternion dq =
        QuatFromAxisAngle(Scale(gyroCorrected, 1.0 / omegaNorm), omegaNorm * dt);
    q = MultiplyQuat(q, dq);
  }
  q = NormalizeQuat(q);
  ...
  {
    const double tempScale = m_hasTemperature
                                 ? 1.0 + m_tempScale * std::abs(m_lastTemperature - m_temperatureAtCal)
                                 : 1.0;
    for (size_t i = 0; i < 3; ++i)
    {
      m_gyroBiasVar[i] =
  ...
  if (m_zUpSolved && !m_forwardSolved)
  {
    ...
    const bool event =
        std::abs(m_motorVoltageDvdt) > m_dvdtThresh || accelLinXY > m_alinThresh;
