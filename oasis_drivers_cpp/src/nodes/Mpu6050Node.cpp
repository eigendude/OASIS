  ...
  : rclcpp::Node(NODE_NAME), m_motorIntent(DEFAULT_EWMA_TAU, DEFAULT_CONDUCTOR_STALE_SECONDS)
{
  ...
  declare_parameter("forward_sign_consistency_samples",
                    imuDefaults.forward_sign_consistency_samples);
  ...
  const double forwardResponseGyroThresh =
      get_parameter("forward_response_gyro_thresh").as_double();
...
bool Mpu6050Node::Initialize()
{
  ...
  m_imuPublisher = create_publisher<sensor_msgs::msg::Imu>(IMU_TOPIC, rclcpp::QoS(10));
  m_debugPublisher =
