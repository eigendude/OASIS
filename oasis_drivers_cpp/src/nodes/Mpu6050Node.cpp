...
void Mpu6050Node::PublishImu()
{
  ...
  const double dt_seconds = m_hasLastSampleTime ? (now - m_lastSampleTime).seconds()
                                               : m_publishPeriod.count();
  ...
  RCLCPP_INFO_THROTTLE(
      ...
      accel_x, accel_y, accel_z, accel_norm, accel_g, bias_diag.accel_norm_mps2,
      bias_diag.accel_g, bias_diag.g_err_mps2, bias_diag.w_conf, bias_diag.var_mag_mps4,
      bias_diag.jerk_avg_mps3, bias_diag.gyro_mag_avg_rads,
      bias_diag.stationary ? "stationary" : "dynamic",
