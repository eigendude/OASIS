...
constexpr double ADC_PHYSICAL_FULL_SCALE_CODES = 27500.0;
...
// ACS37800-DS Rev. 4, Tables 8 through 10: active power sensitivity for the
// 15 A, 30 A, and 90 A variants, referred to the device input
constexpr double POWER_SENSITIVITY_15A_LSB_PER_MILLIWATT = 6.15;
constexpr double POWER_SENSITIVITY_30A_LSB_PER_MILLIWATT = 3.08;
constexpr double POWER_SENSITIVITY_90A_LSB_PER_MILLIWATT = 1.03;
...
double PowerSensitivity(double current_range_amps)
{
  if (current_range_amps == 15.0)
    return POWER_SENSITIVITY_15A_LSB_PER_MILLIWATT;
  if (current_range_amps == 30.0)
    return POWER_SENSITIVITY_30A_LSB_PER_MILLIWATT;
  return POWER_SENSITIVITY_90A_LSB_PER_MILLIWATT;
}
} // namespace
...
double OASIS::PowerMeter::DecodeVoltage(std::uint16_t raw, const Acs37800Config& config)
{
  ValidateAcs37800Config(config);
  const double input_voltage = static_cast<double>(SignExtend(raw, 16)) /
                               ADC_PHYSICAL_FULL_SCALE_CODES * VOLTAGE_INPUT_FULL_SCALE_VOLTS;
  return input_voltage * VoltageMultiplier(config);
}

double OASIS::PowerMeter::DecodeCurrent(std::uint16_t raw, const Acs37800Config& config)
{
  ValidateAcs37800Config(config);
  return static_cast<double>(SignExtend(raw, 16)) / ADC_PHYSICAL_FULL_SCALE_CODES *
         config.current_sense_range_amps;
}

double OASIS::PowerMeter::DecodePower(std::uint16_t raw, const Acs37800Config& config)
{
  ValidateAcs37800Config(config);

  // Tables 8 through 10 specify LSB/mW at the device input. Figures 17 and 18
  // require multiplying by the physical voltage-divider ratio for line power
  const double input_power_milliwatts =
      static_cast<double>(SignExtend(raw, 16)) / PowerSensitivity(config.current_sense_range_amps);
  return input_power_milliwatts / 1000.0 * VoltageMultiplier(config);
}
...
{
  // ACS37800-DS Rev. 4 documents no multi-register snapshot. Read the three
  // volatile registers consecutively to minimize inter-register skew
  const std::uint32_t voltage_current =
      m_transport->ReadRegister(ACS37800_INSTANTANEOUS_VOLTAGE_CURRENT_REGISTER);
  const std::uint32_t power = m_transport->ReadRegister(ACS37800_INSTANTANEOUS_POWER_REGISTER);
  ...
  sample.voltage = DecodeVoltage(static_cast<std::uint16_t>(voltage_current), m_config);
  sample.current = DecodeCurrent(static_cast<std::uint16_t>(voltage_current >> 16U), m_config);
  sample.power = DecodePower(static_cast<std::uint16_t>(power), m_config);
