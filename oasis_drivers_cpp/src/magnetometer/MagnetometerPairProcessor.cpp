...
MagnetometerPairProcessor::MagnetometerPairProcessor(
    IMagnetometerPairSampler& sampler,
...
bool MagnetometerPairProcessor::Initialize()
{
  ...
  covarianceConfig.gate.axis_std_threshold_t = m_processingConfig.stationary_axis_std_threshold_t;
...
bool MagnetometerPairProcessor::Tick(MagnetometerTickOutput& output)
{
      ...
      m_processingConfig.offset_spike_inflation_factor > 1.0)
  {
    if (output.offset_t.norm() >= m_processingConfig.offset_spike_threshold_t)
    {
      ...
      const auto inflationDur = std::chrono::duration_cast<std::chrono::steady_clock::duration>(
          std::chrono::duration<double>(m_processingConfig.offset_spike_inflation_sec));
      m_offsetSpikeInflationUntil = now + inflationDur;
    }
  ...
  {
    MaybeWriteCalibration(output.offset_t, output.covariance_t2, output.covariance_updated, output);
  }
...
{
  const double datasheetNoiseT = m_processingConfig.datasheet_rms_noise_mg * kMilliGaussToTesla;
  const double repeatT = m_processingConfig.set_reset_repeatability_mg * kMilliGaussToTesla;

  const double sigmaRawT = std::sqrt((datasheetNoiseT * datasheetNoiseT) + (repeatT * repeatT));
...
bool MagnetometerPairProcessor::MaybeWriteCalibration(const Eigen::Vector3d& offset_t,
                                                      ...
                                                      MagnetometerTickOutput& output)
{
  ...
  if (m_lastCalibrationWrite)
  {
    const double elapsed =
        std::chrono::duration_cast<std::chrono::duration<double>>(now - *m_lastCalibrationWrite)
            .count();
  ...
  std::filesystem::create_directories(m_calibrationConfig.calibration_path.parent_path(), error);
  if (error)
  {
    output.warning = "Failed to create calibration directory: " + error.message();
    return false;
  }
