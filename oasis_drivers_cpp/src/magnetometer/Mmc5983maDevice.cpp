...
{
  const std::uint8_t setReset =
      mode == MeasurementMode::Set ? kControl0Set : kControl0Reset;
...
bool Mmc5983maDevice::ReadRegisters(std::uint8_t reg, std::uint8_t* values,
...
{
  const std::uint32_t x_raw =
      (static_cast<std::uint32_t>(buffer[0]) << 10) |
      (static_cast<std::uint32_t>(buffer[1]) << 2) |
      ((buffer[6] >> 6) & 0x03);
  const std::uint32_t y_raw =
      (static_cast<std::uint32_t>(buffer[2]) << 10) |
      (static_cast<std::uint32_t>(buffer[3]) << 2) |
      ((buffer[6] >> 4) & 0x03);
  const std::uint32_t z_raw =
      (static_cast<std::uint32_t>(buffer[4]) << 10) |
  ...
  return Eigen::Vector3d{x_signed * m_config.tesla_per_count,
                         y_signed * m_config.tesla_per_count,
