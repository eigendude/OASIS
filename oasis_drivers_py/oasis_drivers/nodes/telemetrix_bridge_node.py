    ...
    def _on_ping_timer(self) -> None:
        ...
        if not self._uptime_ok(uptime_ms):
            ...
            self.get_logger().warning(
                f"Uptime went backwards (previous={previous_uptime_ms} ms, current={uptime_ms} ms)",
            )
            ...
            while rclpy.ok():
                ...
                try:
                    ...
                    if not self._uptime_ok(uptime_ms):
                        ...
                        self.get_logger().warning(
                            f"Uptime went backwards during reconnect (previous={previous_uptime_ms} ms, current={uptime_ms} ms)"
                        )
                        try:
                            bridge.deinitialize()
                        except Exception:
