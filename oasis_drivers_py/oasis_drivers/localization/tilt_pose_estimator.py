...
class TiltPoseEstimator:
    ...
    def _predict_covariance(
        ...
        for idx in range(2):
            delta: np.ndarray = np.zeros(2, dtype=float)
            delta[idx] = epsilon
            f_plus: np.ndarray = f(state + delta, omega)
            f_minus: np.ndarray = f(state - delta, omega)
        ...
        for idx in range(3):
            ...
            delta[idx] = epsilon
            f_plus: np.ndarray = f(state, omega + delta)
            f_minus: np.ndarray = f(state, omega - delta)
