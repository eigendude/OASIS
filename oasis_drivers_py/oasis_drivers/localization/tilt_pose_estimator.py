...
class TiltPoseEstimator:
    ...
    def __init__(
        ...
        self._accel_trust_threshold_mps2 = accel_trust_threshold_mps2
        self._yaw_variance_rad2 = yaw_variance_rad2
        self._min_attitude_variance = min_attitude_variance

        self._gravity_mps2 = 9.80665
    ...
    def update(
        ...
        accel = np.array(accel_values, dtype=float)
        gyro = np.array(gyro_values, dtype=float)

        dt = max(0.0, float(dt_s))
        self._gravity_mps2 = float(calibration.gravity_mps2)

        gyro_cov = self._gyro_covariance(
            angular_velocity_covariance,
            calibration.gyro_bias_cov,
        )
        accel_cov = self._accel_covariance(
            accel,
        ...
        if self._accel_is_trusted(accel):
            ...
            meas_cov = self._orientation_covariance(accel, accel_cov)
    ...
    def _accel_is_trusted(self, accel: np.ndarray) -> bool:
        ...
        threshold = self._accel_trust_threshold_mps2
        return abs(magnitude - self._gravity_mps2) <= threshold

    def _predict_attitude(
        self, gyro: np.ndarray, dt: float
    ) -> tuple[float, float]:
        roll = 0.0 if self._roll is None else float(self._roll)
        pitch = 0.0 if self._pitch is None else float(self._pitch)
    ...
    def _gyro_covariance(
        ...
        base = self._covariance_matrix(angular_velocity_covariance)
        if base is None:
            base = np.zeros((3, 3), dtype=float)

        if gyro_bias_cov.shape != (3, 3):
            bias_cov = np.zeros((3, 3), dtype=float)
        else:
    ...
    def _accel_covariance(
        ...
        base = self._covariance_matrix(linear_accel_covariance)
        if base is None:
            base = np.zeros((3, 3), dtype=float)

        param_cov = self._accel_param_covariance(accel, calibration)
        total = base + param_cov
        return self._symmetrize(total)
    ...
    def _accel_param_covariance(
        ...
        param_cov = jac @ calibration.accel_param_cov @ jac.T
    ...
    def _orientation_covariance(
        ...
        jac = np.vstack((j_roll, j_pitch))

        cov = jac @ accel_cov @ jac.T
    ...
    def _kalman_update(
        ...
        updated_state = np.array(prediction, dtype=float) + k @ innovation
        roll = float(updated_state[0])
        pitch = float(updated_state[1])

        identity = np.eye(2, dtype=float)
        i_minus_k = identity - k

        left_term = i_minus_k @ pred_cov @ i_minus_k.T
        right_term = k @ meas_cov @ k.T
        updated_cov = left_term + right_term
    ...
    def _invert_2x2(self, matrix: np.ndarray) -> np.ndarray:
        ...
        inv_det = 1.0 / det
        inv = np.array([[d, -b], [-c, a]], dtype=float) * inv_det
        return inv

    def _sanitize_covariance(self, covariance: np.ndarray) -> np.ndarray:
        cov = self._symmetrize(covariance)
