...
class PoseLandmarkerNode(rclpy.node.Node):
    def __init__(self) -> None:
        ...
        # Define QoS settings:
        #
        #   - history: 1 corresponds to KEEP_LAST
        #   - depth: 1 (small queue depth)
        #   - reliability: 2 corresponds to BEST_EFFORT
        #   - durability: 2 corresponds to VOLATILE
        #
        qos_settings = {"history": 1, "depth": 1, "reliability": 2, "durability": 2}
        ...
        self._image_sub.subscribe_qos(
            base_topic=self.resolve_topic_name(IMAGE_SUB_TOPIC),
            qos_dict=qos_settings,
        ...
        self._pub = self._image_pub.advertise(
            self.resolve_topic_name(IMAGE_PUB_TOPIC), 10
        )
